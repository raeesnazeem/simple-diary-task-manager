...
const electronStorage = {
  getItem: (name: string): string | null => {
    if (
      ...
      window.electronAPI &&
      window.electronAPI.loadDataSync
    ) {
      try {
  ...
  temporal(
    persist(
      (set) => ({
        ...
        addBlock: (date, type, content = "", index) =>
          set((state) => {
        ...
        mergeBlockWithPrevious: (date, currentIndex) =>
          set((state) => {
        ...
        reorderBlocks: (date, sourceIndex, destinationIndex) =>
          set((state) => {
            const blocks = state.data[date] || []
      ...
      }),
      {
        ...
        merge: (persistedState: any, currentState: DiaryState) => {
          return {
            ...
            autoSyncEnabled:
              persistedState?.autoSyncEnabled ?? currentState.autoSyncEnabled,
          }
        },
      },
    ),
